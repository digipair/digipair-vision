...
[Element].forEach(function (self) {
  ...
  self.prototype.addEventListener = function (type, handle, useCapture) {
    ...
    if (type != 'eventListenerAdded' && type != 'eventListenerRemoved') {
      ...
      let addedEvent = new CustomEvent('eventListenerAdded', {
        detail: eventDetail,
      });
  ...
  self.prototype.removeEventListener = function (type, handle, useCapture) {
    ...
    node.eventListenerList[type] = node.eventListenerList[type].filter(
      function (listener) {
        return (
          ...
          listener.handle &&
          handle &&
          listener.handle.toString() !== handle.toString()
        ); // native event listener API supports empty handles/listeners, but .toString obviously doesn't
      }
    );
    ...
    if (type != 'eventListenerAdded' && type != 'eventListenerRemoved') {
      ...
      let removedEvent = new CustomEvent('eventListenerRemoved', {
        detail: eventDetail,
      });

...
export class Web2VR {
  constructor(container, settings = {}) {
    // main div container
    if (container.nodeType == Node.ELEMENT_NODE) this.container = container;
          ...
          if (rule instanceof CSSStyleRule) {
            ...
            for (const selector of selectors) {
              ...
              if (sel[1] == 'hover') {
                ...
                if (s[0] == '.') rule.selectorText += `, ${s}hover`;
                else if (s[0] == '#')
                  rule.selectorText += `, .${s.substr(1)}hover`;
                else rule.selectorText += `, .${s}hover`;
  ...
  addElement(domElement, parentElement, layer) {
    ...
    if (
      this.settings.ignoreTags.includes(domElement.tagName) ||
      (domElement.classList && domElement.classList.contains('vr-span'))
    )
      return null;
    ...
    this.observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        ...
        for (const node of mutation.removedNodes) {
          ...
          else emptyRemove = true;
        }

        for (const node of mutation.addedNodes)
          this.addElementChildren(
            node,
            mutation.target.element,
            mutation.target.element.layer + 1
          );

        if (!emptyRemove) {
          // when adding new nodes we also need to check for new loaded images
          if (mutation.addedNodes.length > 0) this.allLoadedUpdate();
          else this.update();
        }
      }
    });
  ...
  update() {
    ...
    if (this.updating) {
      ...
      try {
        for (const element of this.elements) element.update();
      } catch (err) {
        console.error(err);
      }
      this.scroll.update();

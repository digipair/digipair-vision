  ...
  tick: function () {
    if (this.el.components['grabbable'].grabbed) {
      ...
      if (this.el.web2vr.scroll.hasScroll) {
        for (const element of this.el.web2vr.elements) element.updateClipping();
      }
      ...
      obj.rotation.y = Math.atan2(
        hand.position.x -
          (obj.position.x + this.el.children[0].getAttribute('width') / 2),
        hand.position.z - obj.position.z
      );

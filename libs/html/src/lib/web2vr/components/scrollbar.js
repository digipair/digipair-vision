AFRAME.registerComponent('vr-scrollbar', {
  init: function () {
    this.el.addEventListener(
      ...
      function (evt) {
        let scrollHeight =
          parseFloat(
            this.el.web2vr.aframe.container.firstElementChild.element.style
              .height
          ) - this.el.web2vr.settings.scrollWindowHeight;
        if (this.el.web2vr.scroll.scrollContainer == this.el.web2vr.container)
          scrollHeight =
            this.el.web2vr.scroll.scrollContainer.scrollHeight -
            this.el.web2vr.scroll.scrollContainer.clientHeight;

        const scrollY =
          ((1 - evt.detail.intersection.uv.y) * scrollHeight) /
          this.el.web2vr.settings.scale;
        ...
        this.el.pointer.object3D.position.setY(
          this.pointEndY +
            evt.detail.intersection.uv.y * (this.pointStartY - this.pointEndY)
        );
  ...
  play: function () {
    const width = parseFloat(
      this.el.web2vr.aframe.container.firstElementChild.getAttribute('width')
    );
    const height = parseFloat(
      this.el.web2vr.aframe.container.firstElementChild.getAttribute('height')
    );
    ...
    this.el.scrollbar.object3D.position.setZ(
      -(this.el.web2vr.settings.layerStep * 4)
    );
    ...
    this.el.pointer.object3D.position.setZ(
      this.el.web2vr.settings.layerStep * 2
    );
  },
});

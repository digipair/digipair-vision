AFRAME.registerComponent('vr-border', {
  ...
  update: function () {
    ...
    else this.borderObject.material.linewidth = borderWidth;

    // custom element border color
    if (this.el.element.borderColor)
      this.borderObject.material.color = this.el.element.borderColor;
    else this.borderObject.material.color = new THREE.Color(this.data.color);
  },
  ...
  tick: function () {
    if (this.running) {
      ...
      this.borderObject.position.set(
        position.x,
        position.y,
        position.z + this.el.element.web2vr.settings.layerStep * 2
      );
      this.running = false;
    }
  },

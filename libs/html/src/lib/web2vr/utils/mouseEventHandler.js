export default class MouseEventHandler {
  ...
  resync() {
    ...
    for (const mouseEvent of mouseEvents) {
      if (
        this.element.domElement.eventListenerList &&
        this.element.domElement.eventListenerList[mouseEvent]
      ) {
  ...
  addMouseListeners(mouseEvents) {
    mouseEvents.forEach((eventName) => {
      this.element.entity.addEventListener(
        eventName,
  ...
  removeMouseListeners(mouseEvents) {
    mouseEvents.forEach((eventName) => {
      this.element.entity.removeEventListener(
        eventName,
        this.mouseEventHandler.bind(this)
      );
    });
  ...
  checkEntityEvents() {
    ...
    for (const mouseEvent of mouseEvents) {
      if (
        this.element.entity.eventListenerList &&
        this.element.entity.eventListenerList[mouseEvent]
      ) {
        this.listeningForMouseEvents = true;
        break;
      }

...
AFRAME.registerComponent('meta-audio-source', {
  schema: {
  ...
  update(oldData: any) {
    ...
    if (oldData.group !== this.data.group) {
      this.resetGroup();
    }
  },
  ...
  createSound() {
    NAF.utils.getNetworkedEntity(this.el).then((networkedEl: any) => {
      ...
      if (ownerId) {
        NAF.connection.adapter
          ...
          .then(() => this.resetGroup())
          .catch((e: any) =>
            console.error(`Error getting media stream for ${ownerId}`, e)
          );
      } else {
  ...
  resetGroup() {
    if (this.sound) {
      const group = this.el.sceneEl
        .querySelector('meta-player [meta-avatar][meta-audio-source]')
        .getAttribute('meta-audio-source').group;
